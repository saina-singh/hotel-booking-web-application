...
CREATE TABLE room_types (
    ...
    CONSTRAINT chk_room_type_mult CHECK (
        base_multiplier > 0
        AND second_guest_multiplier >= 0
    )
);

ALTER TABLE room_types MODIFY code VARCHAR(30) NOT NULL;

ALTER TABLE room_types ADD UNIQUE KEY uq_room_types_code (code);
...
INSERT INTO
    room_types (
        ...
        max_guests,
        base_multiplier,
        second_guest_multiplier
    )
VALUES (1, 'STANDARD', 1, 1.00, 0.00),
...
-- One hotel per city
INSERT INTO
    hotels (city_id, hotel_name)
SELECT city_id, CONCAT('WH ', name)
FROM cities;
...
INSERT INTO
    hotel_capacity_rates (
        hotel_id,
        total_rooms,
        standard_peak_gbp,
        standard_offpeak_gbp
    )
SELECT
    h.hotel_id,
    CASE c.name
...
SELECT hotel_id, 1, ROUND(total_rooms * 0.30)
FROM hotel_capacity_rates
UNION ALL
SELECT hotel_id, 2, ROUND(total_rooms * 0.50)
FROM hotel_capacity_rates
UNION ALL
SELECT hotel_id, 3, total_rooms - ROUND(total_rooms * 0.30) - ROUND(total_rooms * 0.50)
FROM hotel_capacity_rates;

SELECT room_type_id, code, second_guest_multiplier FROM room_types;
...
SELECT
    ...
    inv.rooms_count - COALESCE(SUM(br.rooms_qty), 0) AS rooms_available
FROM
    hotels h
    JOIN cities c ON c.city_id = h.city_id
    ...
    AND NOT(
        b.check_out <= '2025-03-10'
        OR b.check_in >= '2025-03-15'
    )
...
FROM
    hotels h
    JOIN room_types rt ON rt.code IN (
        'SINGLE',
        'DELUXE',
        'SUITE',
        'EXECUTIVE'
...
WHERE
    inv.hotel_id IS NULL;

SELECT COUNT(*) AS rows_found
FROM hotel_room_inventory
WHERE
    room_type_id IN (4, 5, 6, 7);

# Currency conversion at query time
-- Convert booking total from GBP to chosen currency using latest available rate <= today
SELECT b.booking_id, b.booking_code, b.total_gbp, b.currency_code, xr.gbp_to_curr, ROUND(
        b.total_gbp * xr.gbp_to_curr, 2
    ) AS total_in_currency
FROM
    bookings b
    JOIN exchange_rates xr ON xr.currency_code = b.currency_code
    AND xr.rate_date = (
...
WHERE
    email IN (
        'singhsaina11@gmail.com',
        'singhsaina246@gmail.com'
    );

UPDATE users SET role = 'ADMIN' WHERE email = 'singhsaina11@gmail.com';

UPDATE users SET is_active = 1 WHERE email = 'singhsaina11@gmail.com';
...
FROM hotels h
    JOIN features f ON f.name IN (
        'WiFi', 'TV', 'Breakfast', 'Air Conditioning'
    );
...
FROM hotels h
    JOIN features f ON f.name IN (
        'WiFi', 'TV', 'Breakfast', 'Mini-bar', 'Room Service'
    );
...
FROM hotels h
    JOIN features f ON f.name IN (
        'WiFi', 'TV', 'Breakfast', 'Mini-bar', 'Room Service', 'Pool'
    );
...
FROM hotels h
    JOIN features f ON f.name IN (
        'WiFi', 'TV', 'Breakfast', 'Mini-bar', 'Gym'
    );

SELECT h.hotel_name, c.name AS city, rt.code, GROUP_CONCAT(
        f.name
        ORDER BY f.name SEPARATOR ', '
    ) AS features
FROM
    room_type_features rtf
...
SELECT h.hotel_id, rt.code, GROUP_CONCAT(
        f.name
        ORDER BY f.name SEPARATOR ', '
    ) AS features
FROM
...
CREATE TABLE cookie_preferences (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (user_id),
    CONSTRAINT fk_cookie_user FOREIGN KEY (user_id) REFERENCES users (user_id) ON DELETE CASCADE
);
